...
def long_words(words):
    ...
    for word in words:
        if len(word) > 4:
            the_long_words.append(word)

    return the_long_words
...
def halvesies(numbers):
    ...
    return [num/2.00 for num in numbers]
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for index in range(half_list):
        temp_value = items[index - 1]
        items[index - 1] = items[- index]
        items[-index] = temp_value
...
def string_index(string, letter):
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    return [string_index(word, letter) for word in words]
